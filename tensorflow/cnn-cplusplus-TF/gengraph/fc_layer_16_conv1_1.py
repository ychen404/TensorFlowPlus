...
#		pool5_flat = tf.reshape(images_reshaped, [-1, shape])
		...
		fc0l = tf.nn.bias_add(tf.matmul(pool1_flat, fc0w), fc0b)
		fc0 = tf.nn.relu(fc0l)
	...
	with tf.name_scope('fc3') as scope:
		...
		fc12w = tf.Variable (tf.truncated_normal ([102, 102],stddev=1e-2),name='weights')
		fc12b = tf.Variable (tf.constant (1.0, shape=[102], dtype=tf.float32),trainable=True, name='biases')
                fc12l = tf.nn.bias_add (tf.matmul (fc11, fc12w), fc12b, name = 'infer')
		fc12 = tf.nn.relu(fc12l)
	        fc12 = tf.nn.dropout(fc12, 0.5)
        
        with tf.name_scope('fc13') as scope:
		fc13w = tf.Variable (tf.truncated_normal ([102, 102],stddev=1e-2),name='weights')
		fc13b = tf.Variable (tf.constant (1.0, shape=[102], dtype=tf.float32),trainable=True, name='biases')
                fc13l = tf.nn.bias_add (tf.matmul (fc12, fc13w), fc13b, name = 'infer')
		fc13 = tf.nn.relu(fc13l)
	        fc13 = tf.nn.dropout(fc13, 0.5)
        
        with tf.name_scope('fc14') as scope:
		fc14w = tf.Variable (tf.truncated_normal ([102, 102],stddev=1e-2),name='weights')
		fc14b = tf.Variable (tf.constant (1.0, shape=[102], dtype=tf.float32),trainable=True, name='biases')
                fc14l = tf.nn.bias_add (tf.matmul (fc13, fc14w), fc14b, name = 'infer')
		fc14 = tf.nn.relu(fc14l)
	        fc14 = tf.nn.dropout(fc14, 0.5)
        
        with tf.name_scope('fc15') as scope:
		fc15w = tf.Variable (tf.truncated_normal ([102, 102],stddev=1e-2),name='weights')
		fc15b = tf.Variable (tf.constant (1.0, shape=[102], dtype=tf.float32),trainable=True, name='biases')
                fc15l = tf.nn.bias_add (tf.matmul (fc14, fc15w), fc15b, name = 'infer')
#		fc15 = tf.nn.relu(fc15l)
...
#	train_op = optimizer.minimize (loss, global_step=global_step, name="train")
	...
	tf.train.write_graph (sess.graph_def, "models/", "graph_fc_16_conv1.pb", as_text=False)
