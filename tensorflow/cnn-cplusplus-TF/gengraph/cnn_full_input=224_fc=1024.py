...
with tf.Session() as sess:
	...
	pool5 = tf.nn.max_pool (conv5_1,
		...
		fc0w = tf.Variable(tf.truncated_normal([shape, 1024],stddev=1e-2), name='weights')
		fc0b = tf.Variable(tf.constant(1.0, shape=[1024],dtype=tf.float32),trainable=True, name='biases')
	...
	with tf.name_scope('fc1') as scope:
		...
		fc1w = tf.Variable(tf.truncated_normal([1024, 1024],stddev=1e-2), name='weights')
	...
	with tf.name_scope('fc2') as scope:
		fc2w = tf.Variable (tf.truncated_normal ([1024, 102],stddev=1e-2),name='weights')
	...
	tf.train.write_graph (sess.graph_def, "models/", "graph_full_224_fc_1024.pb", as_text=False)
