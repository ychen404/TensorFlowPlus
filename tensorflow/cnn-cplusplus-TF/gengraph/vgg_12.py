...
with tf.Session() as sess:
    ...
    with tf.name_scope("fc6") as scope:
        ...
        fc6w = tf.Variable(tf.truncated_normal([shape, 4096],stddev=1e-2), name='weights')
        fc6b = tf.Variable(tf.constant(1.0, shape=[4096],dtype=tf.float32),trainable=True, name='biases')
        pool4_flat = tf.reshape(pool4, [-1, shape])
        fc6l = tf.nn.bias_add(tf.matmul(pool4_flat, fc6w), fc6b)
        fc6 = tf.nn.relu(fc6l)
        fc6 = tf.nn.dropout(fc6, 0.5)

    with tf.name_scope("fc7") as scope:
        fc7w = tf.Variable(tf.truncated_normal([4096, 4096],stddev=1e-2), name='weights')
        fc7b = tf.Variable(tf.constant(1.0, shape=[4096],dtype=tf.float32),trainable=True, name='biases')
        fc7l = tf.nn.bias_add(tf.matmul(fc6, fc7w), fc7b)
        fc7 = tf.nn.relu(fc7l)
        fc7 = tf.nn.dropout(fc7, 0.5)

    with tf.name_scope("fc8") as scope:
        fc8w = tf.Variable(tf.truncated_normal([4096, 102],stddev=1e-2), name='weights')
    ...
    tf.train.write_graph (sess.graph_def, "models/", "graph_vgg12.pb", as_text=False)
