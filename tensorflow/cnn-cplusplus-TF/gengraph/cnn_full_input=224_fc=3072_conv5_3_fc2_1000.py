...
with tf.Session() as sess:
    ...
    with tf.name_scope("fc0") as scope:
        ...
        fc0w = tf.Variable(tf.truncated_normal([shape, 3072],stddev=1e-2), name='weights')
        fc0b = tf.Variable(tf.constant(1.0, shape=[3072],dtype=tf.float32),trainable=True, name='biases')
    ...
    with tf.name_scope('fc1') as scope:
        ...
        fc1w = tf.Variable(tf.truncated_normal([3072, 3072],stddev=1e-2), name='weights')
        fc1b = tf.Variable(tf.constant(1.0, shape=[3072],dtype=tf.float32),trainable=True, name='biases')
        fc1l = tf.nn.bias_add(tf.matmul(fc0, fc1w), fc1b)
        fc1 = tf.nn.relu(fc1l)

    with tf.name_scope('fc2') as scope:
        fc2w = tf.Variable (tf.truncated_normal ([3072, 1000],stddev=1e-2),name='weights')
    ...
    tf.train.write_graph (sess.graph_def, "models/", "graph_full_224_fc_3072_conv5_3_fc2_1000.pb", as_text=False)
