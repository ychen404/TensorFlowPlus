...
#
	with tf.name_scope('fc1') as scope:
		...
		fc1b = tf.Variable(tf.constant(1.0, shape=[4096], dtype=tf.float32),
								trainable=True, name='biases')
		pool5_flat = tf.reshape(pool1, [-1, shape])
	...
	#loss = tf.reduce_mean (cross_entropy, name='loss')
    	loss = tf.reduce_sum (tf.square (fc3l - oneHot), name="loss")
...
#	train_op = optimizer.minimize (loss, global_step=global_step, name="train")
	...
	tf.train.write_graph (sess.graph_def, "models/", "graph_saman.pb", as_text=False)
