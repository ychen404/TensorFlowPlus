...
OUTPUT = 64
NUM_CLASSES = OUTPUT
STR = "graph_conv_16_bat_" + str(BATCH_SIZE) + "_" + "w_" + str(OUTPUT) + ".pb"
...
with tf.Session() as sess:
    ...
    with tf.name_scope('conv_15') as scope:
        ...
        biases = tf.Variable(tf.constant(0.0, shape=[OUTPUT], dtype=tf.float32),
                                 trainable=True, name='biases')
        conv_15 = tf.nn.bias_add(conv, biases)
   
    with tf.name_scope('conv_16') as scope:
        kernel = tf.Variable(tf.truncated_normal([3, 3, OUTPUT, OUTPUT], dtype=tf.float32,
                                stddev=1e-2), name='weights')
        conv = tf.nn.conv2d(conv_15, kernel, [1, 1, 1, 1], padding='SAME')
        biases = tf.Variable(tf.constant(0.0, shape=[OUTPUT], dtype=tf.float32),
                                 trainable=True, name='biases')
        conv_16 = tf.nn.bias_add(conv, biases)
