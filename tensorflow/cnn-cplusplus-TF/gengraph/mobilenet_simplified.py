...
#        print "The shape of conv_2_1_dw is" + str(conv_2_1_dw.get_shape())
    ...
    with tf.name_scope('conv_2_2') as scope:
        print "conv_2_2"
        kernel = tf.Variable(tf.truncated_normal([1,1,3,64], dtype=tf.float32, stddev=1e-2), name='weights')
...
#        conv_2_2 = tf.nn.relu(out)
        conv_2_2 = conv
        print "The shape of conv_2_2 is " + str(conv_2_2.get_shape())
     #   flat = tf.reshape(conv_2_2, [BATCH_SIZE, -1])
...
##       print "shape " + str(shape)
    ...
    loss = tf.reduce_mean (tf.square(tf.subtract(conv_2_2, oneHot)), name='loss')
    print loss
#    pdb.set_trace()
