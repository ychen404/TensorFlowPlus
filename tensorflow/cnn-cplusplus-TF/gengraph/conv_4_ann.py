...
with tf.Session() as sess:
    ...
    # change the labels placeholder shape to (BATCH_SIZE, NUM_CLASSES) so that don't need to use oneHot encoder which is not supported in TFLite -- 8/15
    labels_placeholder = tf.placeholder (tf.float32, shape=(BATCH_SIZE, NUM_CLASSES), name="label")
...
#    pdb.set_trace()
    ...
    diff = tf.subtract(out_0, labels_placeholder)
    tmp = tf.multiply(diff, diff)
    loss = tf.reduce_mean (tmp, name='loss')
    #loss = tf.reduce_mean (tf.square(tf.subtract(out_0, labels_placeholder)), name='loss')
    #pdb.set_trace()
